...
Æxyr (ASH-er) is a self-hosted autonomous AI agent platform that lives inside a Docker container on your hardware. Built on the Agent Zero Framework, It operates its own Linux environment with scoped access to a terminal, file system, package management, and service orchestration — governed by a biological-inspired safety protocol that gates every action through confidence, safety, and necessity scoring before execution.
